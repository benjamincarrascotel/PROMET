...
class TransporteController extends Controller
{
    ...
    public function cambio_fase_create($id){
        ...
        if($activo->arriendo_flag && !$activo->venta_flag)
            $proceso = ArriendoActivo::where('activo_id', $id)->whereNotIn('estado', ["TERMINADO", "CAMBIO DE PROCESO"])->first();
        elseif($activo->venta_flag && !$activo->arriendo_flag)
            $proceso = Venta::where('activo_id', $id)->whereNotIn('estado', ["TERMINADO", "CAMBIO DE PROCESO"])->first();
        else{
